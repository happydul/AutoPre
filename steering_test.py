...
def main():
    # load data
    if not os.path.exists('./steering_test'):
        os.mkdir('./steering_test')
    ...
    for epoch in range(EPOCH):
        for step, img in enumerate(data_loader):
            ...
            if step == 0:
               ...
               # print b_wait.size()
                ...
                if epoch == EPOCH:
                    save_image(pic, './steering_test/image_{}.jpg'.format(step))
            if step > 0:
                encoded, decoded = auto_encoder(b_wait)
                loss = loss_function(decoded, b_x)
                b_wait = b_x
        ...
        torch.save(auto_encoder.state_dict(), './steering_test.pth')
        torch.save(auto_encoder, 'steering_test.pkl')
